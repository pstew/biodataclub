...
**Background**: We are in our daily work, as a bioinformatician, biostatistician or as scientists in general, relying on other researchers tools and databases, i.e. “standing on the shoulders of giants”. We use these tools to simplify our work, but also to make sure that we do the best and correct analysis. One of the challenges is the vast number of new tools published every year with only a few surviving the test of time. A first pass to find out if a new tool is useful and reliable takes about a day and requires testing it with real data.
...
**Background**: The information about cancerous tumors can be recorded at different scales and resolutions in both space and time. The resulting data may include multi-omics information, radiographs, histopathology images, clinical notes, and demographic data. Jointly learning from such multi-modal, heterogeneous information with the possibility of missing data samples and modalities is challenging but crucial for understanding cancer. This project is the first step towards building general purpose multi-modal models that will integrate various modalities and answer clinical questions.

This hackathon project aims at building graphs (consisting of nodes and edges) from various (two or more) omics datasets and then quantitatively analyze the relationships between nodes / subgraphs to unravel the underlying biological phenomena (such as cancer) captured by multiple omics data. The project will start with feature engineering involving (but not limited to) removal of redundant features, dimensionality reduction, collinear analysis, data imputation etc. We will then identify and define the entities such as nodes, edges, node features, edge features, sub-graphs. We will develop graph adjacencies and corresponding graphs from the given data modalities. After building initial graphs and estimating their features for each dataset, we will use Graph Neural Networks (GNNs) to finetune the structure and features of these graphs for the specified outcome, e.g., overall survival. The project will also involve building visualization at each stage of data processing. The team will share pre-processed and curated datasets, machine learning models, and basic visualization codes. During the hackathon, a web-based visualization tool and project documentation will also be developed. The finalized pipeline will aim to enable its users to enter any multi-omics data, generate graphs, carry out relational learning using GNNs to predict the downstream task.

**Technical Specifications**: Our main workhorse will be Python and PyTorch deep learning framework. We will be extensively using PyTorch-Geometric library, CUDA / cuDNN, pandas, numpy, matplotlib, seaborn, Scikit-learn, and networkx packages. Familiarity with Graphical Neural Networks, high-dimensional omics data, feature engineering, Github.io, and devdoc.io is preferable.

**Programming background**: Python, PyTorch, torch-geometric, python virtual environments, and Jupyter notebooks. We are willing to accommodate someone who does not have required programming background but is able to provide meaningful contribution by bringing in applicable knowledge to improve the outlook of our project, especially people having experience in developing API, and project documentation.
